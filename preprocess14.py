...
url = 'https://raw.githubusercontent.com/mdcollab/covidclinicaldata/master/data/04-14_carbonhealth_and_braidhealth.csv'
...
dataframe = dataframe.drop(columns=['cough_severity', 'sob', 'sob_severity', 'pulse', 'sys', 'dia', 'rr', 'sats', 'ctab'])
dataframe = dataframe.drop(columns=['labored_respiration', 'diarrhea', 'loss_of_smell', 'loss_of_taste'])
dataframe = dataframe.fillna(0)
dataframe = dataframe*1
dataframe['covid19_test_results'] = dataframe['covid19_test_results'].eq('Positive').mul(1)
dataframe.to_csv('../TrainingData/processed_data14.csv')
print(dataframe)